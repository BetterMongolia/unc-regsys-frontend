...
const SelectCategory = ({
  ...
  const [selectedComps, setSelectedComps] = React.useState<any>([]);
  const selectInputRef = React.useRef<any>();
  const toast = useToast();
  //   console.log(compId, sportId);
  const getCategories = async () => {
    ...
    let res;
    if (!isSelectedTeam)
      res = await apiSports.getAvailableCategory(
        eventId,
        competitionId,
        `athleteId=${id}`
      );
    ...
    if (res.success) {
      const tempData = res?.data.map(({ name, _id }: any) => {
  ...
  const onSubmit = async () => {
    ...
    let res;
    if (!isSelectedTeam) {
      res = await apiComps.registerComp(eventId, competitionId, {
        data: [
          {
            athleteId: id,
            categories: selectedComps,
            coach: coachIds,
          },
        ],
      });
    ...
    <>
      <Box pr="10px">
        <Select
          ...
          placeholder="Тэмцээний ангилал сонгох..."
          noOptionsMessage={() => (
      ...
      <Button onClick={onSubmit} isDisabled={selectedComps.length === 0}>
        <chakra.svg
          width="16px"
          height="16px"
          minW="16px"
