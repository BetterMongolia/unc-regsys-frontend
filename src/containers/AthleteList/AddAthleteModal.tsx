...
const AddAthleteModal = ({ isOpen, onClose, selectedAthlete }: any) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      image: Yup.mixed()
        ...
        .required("Заавал файл оруулна уу"),
      birthday: Yup.date()
    ...
    onSubmit: async (values: any) => {
      ...
      if (values.image) {
        ...
        if (result.success) {
          ...
          const res = await apiAthletes.addAthlete(tempValues);
          if (res.success) {
            if (res.message) {
              toast({
                title: "Алдаа!",
  ...
  React.useEffect(() => {
    ...
    if (selectedAthlete) {
      ...
      if (selectedAthlete.birthday) {
        formik.setFieldValue(
          "birthday",
          selectedAthlete.birthday.split("T")[0]
        );
      }
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            >
              ...
              >
                ...
                {/* </Flex> */}
                {/* <Box w="100%" d="flex" justifyContent="space-between"></Box> */}
              </FormControl>
            </Box>
            {/* <Button
              fontSize="13px"
