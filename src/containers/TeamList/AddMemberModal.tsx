...
const AddMember = ({
    ...
    if (teamID === null) {
      ...
      data
        .filter((el: any, index: number) =>
          member[0]?.members.map((item: any) => {
            el.id === item?.member;
          })
        )
        .map((item: any, index: number) => {
          tmp.push({ ...item, role: member[0]?.members[index]?.role });
        });
  ...
  // useEffect(() => {
  //   const tmpSelect = data.map((person: any) => ({ member: person._id }));
  //   setAthlethes(tmpSelect);
                  ...
                  <tbody>
                    {data?.map((d: any, i: number) => (
                      <tr key={`tr-${i}`}>
                        <td>
                          <CustomCheckbox
                            check={selectedAthlete.map(
                              (el: any) => d?.id === el?.id
                            )}
                            onChange={(checked: boolean) => {
                              let updatedList = [...selectedAthlete];
                              if (checked) {
                                for (var i = 0; i < updatedList.length; i++) {
                                  if (updatedList[i]?.id === d?.id) {
                                    return;
                                  }
                                }
                                updatedList.push(d);
                              ...
                              setSelectedAthlete(updatedList);
                              // if (checked) {
                              //   const found = selectedAthlete.find(
                              //     (element: any) => element.id === d.id
                              //   );
                              ...
                              //   if (found) {
                              //   } else {
                              //     let tmp = [...selectedAthlete, d];
                              //     setSelectedAthlete(tmp);
                              //   }
                              // } else {
                              //   const tmp = selectedAthlete?.findIndex(
                              //     (a: any) => a.id === d?.id
                              //   );
                              //   let tmpAthlete = [...selectedAthlete];
                              //   if (tmpAthlete.length === 1) {
                              //     tmpAthlete = [];
                              //   } else {
                              //     tmpAthlete.splice(tmp, 1);
                              //   }
                              //   setSelectedAthlete(tmpAthlete);
                              // }
                            }}
                            index={i}
                          />
