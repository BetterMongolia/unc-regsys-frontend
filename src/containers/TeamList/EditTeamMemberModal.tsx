...
const EditTeamMemberModal = ({ isOpen, onClose, id, name, athletes }: any) => {
      ...
      Cell: ({ row }: any) => (
        <CheckboxGroup>
          <Checkbox
            ...
            onChange={() => {
              // console.log("wtf", selectedAthlete, row);
              if (selectedAthlete.includes(row.original)) {
                selectedAthlete.splice(row?.index, 1);
                setSelectedAthlete(selectedAthlete);
              } else
                setSelectedAthlete((prevState: any) => [
                  ...prevState,
                  row.original,
                ]);
            }}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      // const res = await apiTeams.addAthletes(values);
